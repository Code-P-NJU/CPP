...
void MainScene::initScene()
{
    ...
    ui.kill_count->setFont(QFont("MS Serif",30,100,1));
    ui.level_point->setFont(QFont("MS Serif", 15, 100, 1));
...
void MainScene::game_main() 
{
    ...
    for (int i = 0; i < enemies_1.size(); ++i)//enemy_1的行为
    {
        ...
        {
            player->hp -= 1 + enemies_1[i]->get_level();//默认伤害为1
            if (player->hp <= 0)
            {
                dead_game_over();
            }
...
void MainScene::enemy_spown() 
{
    ...
    if (x == 0)
    {
        enemy_1* p = new enemy_1(1, 5, rand() % (GAME_HEIGHT - 55), 10, enemy_level, ENEMY_SPEED + enemy_level / 2);
        enemies_1.append(p);//后期更改难度时要改参数
    }
    else if (x == 1)
    {
        enemy_1* p = new enemy_1(1, rand() % (GAME_WIDTH - 55), 5, 10, enemy_level, ENEMY_SPEED + enemy_level / 2);
        enemies_1.append(p);//后期更改难度时要改参数
    }
    else if (x == 2)
    {
        enemy_1* p = new enemy_1(1, GAME_WIDTH - 55, rand() % (GAME_HEIGHT - 55), 10, enemy_level, ENEMY_SPEED + enemy_level / 2);
        enemies_1.append(p);//后期更改难度时要改参数
    }
    else if (x == 3)
    {
        enemy_1* p = new enemy_1(1, rand() % (GAME_WIDTH - 55), GAME_HEIGHT - 55, 10, enemy_level, ENEMY_SPEED + enemy_level / 2);
        enemies_1.append(p);//后期更改难度时要改参数
    }
    if (y == 0)
    {
...
MainScene::MainScene(QWidget* parent) : QWidget(parent), seed(std::time(0))
{
    ui.setupUi(this);
    initScene(); //初始化场景
    connect(&total_timer, &QTimer::timeout, [&]()
        {
            if (time > 0)
            {
                --time;
...
void MainScene::StartGame(int id) 
{   
    ui.HP_up->disconnect();
    ui.Lv_up->disconnect();
    ui.Speed_up->disconnect();
